...
- Lab 14 startup now defers at most once to `DOMContentLoaded` and fails visibly if an inherited runtime is unavailable, eliminating the unbounded zero-delay polling loop that could starve browser lifecycle events.
...
- wait for the final interactive-app contract rather than treating the incidental `window.load` milestone as application readiness.
