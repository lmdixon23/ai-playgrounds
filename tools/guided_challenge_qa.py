...
def check_generic(page,slug,checks):
    ...
    if has_r4(page):checks.append(('language_switch_delegated_to_r4_qa',True,{'reason':'R4 runtime active'}))
    else:
        switch_native_locale(page,'zh');values_zh=page.locator('[data-guided-field]').evaluate_all('(els)=>els.map(e=>e.value)')
        checks.append(('language_switch_preserves_locked_prediction',values_zh==values_before and state(page)=='locked',{'before':values_before,'after':values_zh}));switch_native_locale(page,'en')
    ...
    checks.append(('mechanism_specific_actual_text',semantic,{'slug':slug,'actual':actual[:700]}));checks.append(('result_unconcealed_after_reveal',page.locator('[data-guided-concealed="1"]').count()==0,{}))
    page.locator('.suite-guided-compare').click();wait_state(page,'compared');checks.append(('compare_precedes_explanation',not page.locator('.suite-guided-explain-wrap').is_hidden(),{}))
    exp=page.locator('[data-guided-explanation]');exp.fill('The mechanism differed because the ranking or update rule changed.');checks.append(('transfer_requires_explanation',not page.locator('.suite-guided-transfer').is_disabled(),{}))
    page.locator('.suite-guided-transfer').click();wait_state(page,'awaiting-prediction')
    cleared=page.locator('[data-guided-field]').evaluate_all('(els)=>els.every(e=>e.value==="")');checks.append(('transfer_requires_new_prediction',bool(cleared),{}))
    ...
    page.locator('.suite-guided-reset').click();wait_state(page,'inactive');hist=history(page);checks.append(('reset_returns_inactive_and_records_reset',state(page)=='inactive' and 'reset' in hist,{'history':hist}))
...
def check_knn(page,checks):
    ...
    page.locator('[data-guided-class="A"]').click();page.wait_for_timeout(100)
    native_ready=not page.locator('#guidedLock').is_disabled();status=page.locator('#guidedStatus').inner_text().strip()
    checks.append(('knn_exact_neighbor_prediction_complete',native_ready,{'k':k,'status':status,'suite_state':state(page)}))
    if not native_ready:raise RuntimeError(f'KNN native prediction did not complete: {status}')
    if has_r4(page):checks.append(('knn_language_switch_delegated_to_r4_qa',True,{'reason':'R4 runtime active; R3 certifies native EN/ZH locked prediction'}))
    else:
        switch_native_locale(page,'zh');checks.append(('language_switch_preserves_knn_prediction',not page.locator('#guidedLock').is_disabled(),{'suite_state':state(page)}));checks.append(('knn_extension_translates_to_chinese',page.locator('.suite-guided-knn-compare').inner_text().strip()=='比较',{'label':page.locator('.suite-guided-knn-compare').inner_text().strip()}));switch_native_locale(page,'en');checks.append(('knn_extension_returns_to_english',page.locator('.suite-guided-knn-compare').inner_text().strip()=='Compare',{'label':page.locator('.suite-guided-knn-compare').inner_text().strip()}))
    page.locator('#guidedLock').click();wait_state(page,'locked');checks.append(('knn_reveal_enabled_after_lock',not page.locator('#guidedReveal').is_disabled(),{}))
    page.locator('#guidedReveal').click();wait_state(page,'revealed');result=page.locator('#guidedResult').inner_text().strip();checks.append(('knn_reveal_compares_neighbors',bool(result) and '/' in result,{'result':result[:220]}))
    page.locator('.suite-guided-knn-compare').click();wait_state(page,'compared');exp=page.locator('[data-knn-guided-explain]');exp.fill('The distance rule changed which points ranked closest.');checks.append(('knn_transfer_requires_explanation',not page.locator('.suite-guided-knn-transfer').is_disabled(),{}))
    metric_before=page.locator('#metricSel').input_value();page.locator('.suite-guided-knn-transfer').click();wait_state(page,'awaiting-prediction');metric_after=page.locator('#metricSel').input_value();checks.append(('knn_transfer_changes_closeness_rule',metric_before!=metric_after,{'before':metric_before,'after':metric_after}))
    ...
    with local_repo_server() as origin,sync_playwright() as p:
        ...
        try:
            for slug in SLUGS:
                ...
                try:
                    response=page.goto(f'{origin}/playgrounds/{slug}/index.html',wait_until='domcontentloaded',timeout=NAV_TIMEOUT_MS);checks.append(('local_artifact_http_200',response is not None and response.status==200,{} if response is None else {'status':response.status}));page.wait_for_function('() => !!window.__suiteGuidedChallenge',timeout=STATE_TIMEOUT_MS)
                    ...
                    try:diagnostic['knn_status']=page.locator('#guidedStatus').inner_text().strip() if slug=='knn-classifier' else None
