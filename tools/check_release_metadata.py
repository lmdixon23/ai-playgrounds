...
def main() -> int:
    ...
    require("10.5281/zenodo.21854217" not in cff, "Archived v1.0.1 DOI must not be attached to v1.7.2")
    require(f"releases/tag/{TAG}" in readme and "AI Playgrounds v1.7.2 is the current software release" in readme, "README current-release boundary mismatch")
    ...
    for index_name in ("CHANGELOG.md", "RELEASE_NOTES.md"):
        ...
        for version in ("1.6.1", "1.6.2", "1.7.0", "1.7.1", "1.7.2"):
    ...
    required_files = (
        ...
        ".github/workflows/publish-v1.7.2.yml",
    )
    ...
    publisher_workflow = read(".github/workflows/publish-v1.7.2.yml")
    require('workflows: ["Verify"]' in deploy_workflow and "build_site_v1_7_2.py" in deploy_workflow, "Pages deployment is not chained to Verify and v1.7.2 composition")
    require("TAG: v1.7.2" in publisher_workflow and "test_v1_7_2_public_release.py" in publisher_workflow, "v1.7.2 publisher boundary is incomplete")
    ...
    require("**Release:** v1.7.2" in ANALYTICS_SPEC.read_text(encoding="utf-8"), "Analytics specification is not rebound to v1.7.2")
    ...
    require(all(r.get("status") == "active" for r in v2), "v1.7.2 requires all 15 Quick Assigns active")
    require(all(r.get("locales") == ["en", "zh", "vi", "es"] for r in v2), "v1.7.2 Quick Assign locale contract changed")
