...
def main() -> int:
    ...
    require("10.5281/zenodo.21854217" not in cff, "Archived v1.0.1 DOI must not be attached to v1.7.0")
    require(f"releases/tag/{TAG}" in readme and "AI Playgrounds v1.7.0 is the current software release" in readme, "README current-release boundary mismatch")
    require("Archived v1.0.1 DOI" in readme and "10.5281/zenodo.21854217" in readme, "README historical DOI boundary missing")
    require(RELEASE_170.is_file() and "# AI Playgrounds v1.7.0" in RELEASE_170.read_text(encoding="utf-8"), "v1.7.0 release notes missing")
    ...
    required_files = (
        ...
        "tools/build_site_v1_7_final.py", "tools/build_site_v1_7.py", "tools/test_v1_6_1_quick_assign_currency.py",
        "tools/test_v1_6_1_design_consistency.py", "tools/test_v1_6_2_public_provenance.py", "tools/test_v1_7_all_quick_assigns.py",
        "tools/test_v1_7_public_release.py", "tools/test_minimax_alpha_beta.py", "tools/test_transformer_language_model.py",
        "tools/test_agent_tool_context.py",
    )
    ...
    require("**Release:** v1.7.0" in ANALYTICS_SPEC.read_text(encoding="utf-8"), "Analytics specification is not rebound to v1.7.0")
    ...
    require(all(r.get("status") == "active" for r in v2), "v1.7.0 requires all 15 Quick Assigns active")
    require(all(r.get("locales") == ["en", "zh", "vi", "es"] for r in v2), "v1.7.0 Quick Assign locale contract changed")
