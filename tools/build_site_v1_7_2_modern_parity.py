...
"""v1.7.2 parity wrapper for Labs 13-15.
...
SHELL_LABELS = {
    "en": {"back":"Back to all playgrounds", "theme":"Theme", "reset":"Reset lab"},
    "zh": {"back":"返回全部 playground", "theme":"主题", "reset":"重置实验"},
    "vi": {"back":"Quay lại tất cả playground", "theme":"Giao diện", "reset":"Đặt lại lab"},
    "es": {"back":"Volver a todos los playgrounds", "theme":"Tema", "reset":"Reiniciar laboratorio"},
}

STYLE = r'''<style id="v172-modern-parity-style">
...
def tldr(data: dict, slug: str) -> str:
    ...
    return f'''<aside class="ap-modern-tldr" aria-labelledby="ap-modern-big-label">
<strong id="ap-modern-big-label" {loc_attr(data,'biglabel')}>{html_lib.escape(chrome['biglabel'])}</strong>
...
<div>© 2026 Logan M. Dixon · <a href="https://lmdixon23.github.io/" {loc_attr(data,'portfolio')}>{html_lib.escape(c['portfolio'])}</a> · <a href="../../index.html">AI Playgrounds</a> · <a href="../../curriculum.html">Curriculum</a> · <a href="../../teacher-pack.html">Teacher Pack</a> · <a href="https://github.com/lmdixon23/ai-playgrounds">Source</a> · MIT · <a href="../../research-and-citation.html">Citation</a> · <a href="https://github.com/lmdixon23/ai-playgrounds/issues" {loc_attr(data,'report')}>{html_lib.escape(c['report'])}</a> · <a href="https://orcid.org/0009-0001-0592-462X">ORCID</a> · v1.7.2</div>
</footer>'''
...
def runtime(data: dict, slug: str) -> str:
    ...
    return f'''<script id="v172-modern-parity-runtime">
...
def patch(slug: str, data: dict) -> None:
    ...
    if 'id="v172-modern-parity-style"' in page:
...
def validate() -> None:
    for slug in MODERN:
        ...
        surface_count = len(re.findall(r'<[^>]+\bdata-quick-assign-id\s*=', page, flags=re.I))
        if surface_count != 1:
            raise RuntimeError(
                f"Modern parity changed Quick Assign surface count: {slug} ({surface_count})"
            )
    core.validate_local_references()


def build_predecessor() -> None:
    """Build v1.7.1 in an isolated clean workspace.

    Historical builders intentionally monkeypatch predecessor modules while
    composing old release layers and write intermediate release evidence beside
    the source tree. A fresh workspace isolates both kinds of state, so the
    v1.7.2 composition is repeatable even after another release build.
    """
    ignored = shutil.ignore_patterns(
        ".git", "_site", "release-evidence", "__pycache__", ".pytest_cache", "node_modules"
    )
    with tempfile.TemporaryDirectory(prefix="ai-playgrounds-v171-") as temp_dir:
        isolated_root = Path(temp_dir) / "repo"
        shutil.copytree(ROOT, isolated_root, ignore=ignored)
        run = subprocess.run(
            [sys.executable, str(isolated_root / "tools" / "build_site_v1_7_1.py")],
            cwd=isolated_root,
            text=True,
            capture_output=True,
            check=False,
        ...
        if run.returncode:
            detail = run.stderr.strip() or run.stdout.strip() or "v1.7.1 predecessor build failed"
            raise RuntimeError(detail)

        isolated_site = isolated_root / "_site"
        if not isolated_site.is_dir():
            raise RuntimeError("v1.7.1 predecessor build did not create _site")
        if SITE.exists():
            shutil.rmtree(SITE)
        shutil.copytree(isolated_site, SITE)
...
def build_site() -> None:
    ...
    for slug in MODERN:
        patch(slug, data)
    validate()
    print("Built v1.7.2 modern-lab parity candidate without changing AI algorithms or assignment state")
