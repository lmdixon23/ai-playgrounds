...
FOOTER = r'''
<footer class="lab14-provenance" data-lab14-v14-provenance>
...
function installLanguageSelect(){
  ...
  bar.appendChild(select);select.value=locale();select.addEventListener('change',()=>{window.Lab14Localization.setLocale(select.value);updateUrl(select.value)});
}
...
function eventMessage(event,state,decision){
...
function renderJourney(){
  ...
  if($('#lab14-journey-status'))$('#lab14-journey-status').textContent=eventMessage(event,state,decision);
}
...
function init(){
  ...
  const state=$('#stateText');if(state)new MutationObserver(()=>renderJourney()).observe(state,{subtree:true,childList:true,characterData:true});
...
def build_public(output: Path) -> Path:
    ...
    required = (
        'id="lab14-v14-experience-runtime"',
        'id="lab14-v14-experience-style"',
        'data-lab14-v14-provenance',
