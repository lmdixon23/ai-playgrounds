...
This layer gives Labs 13-15 the mature suite's open keyboard/text-state
accessibility panel and localized Quick Assign accessible names. It mirrors the
existing stateText/textState output and does not create a second simulation
state. Runtime synchronization is intentionally independent of Quick Assign
creation timing so accessibility state cannot stall while another surface is
initializing.
"""
...
import build_site_v1_7_2_modern_parity_complete as base
...
A11Y = {
    "en": {
        "summary":"♿ Text and keyboard support", "title":"Text and keyboard support",
        ...
        "keyboard":"Keyboard path", "k1":"Use Tab and Shift+Tab to move through controls.",
        ...
        "state":"Text state summary", "state_wait":"Current text state is preparing…",
    ...
    "vi": {
        ...
        "keyboard":"Lộ trình bàn phím", "k1":"Dùng Tab và Shift+Tab để di chuyển giữa các điều khiển.",
        ...
        "state":"Tóm tắt trạng thái dạng văn bản", "state_wait":"Đang chuẩn bị trạng thái văn bản hiện tại…",
    ...
    "es": {
        "summary":"♿ Soporte de texto y teclado", "title":"Soporte de texto y teclado",
        ...
        "keyboard":"Recorrido con teclado", "k1":"Usa Tab y Shift+Tab para desplazarte entre los controles.",
        ...
        "state":"Resumen textual del estado", "state_wait":"Preparando el estado textual actual…",
...
STYLE = r'''<style id="v172-modern-a11y-parity-style">
...
def attrs(values: dict[str, str]) -> str:
    return " ".join(f'data-ap-{locale}="{html_lib.escape(str(text), quote=True)}"' for locale, text in values.items())
...
<section class="ap-modern-a11y-card" aria-labelledby="ap-modern-a11y-keyboard-title"><h3 id="ap-modern-a11y-keyboard-title" {a11y_attrs("keyboard")}>{html_lib.escape(en["keyboard"])}</h3><ul><li {a11y_attrs("k1")}>{html_lib.escape(en["k1"])}</li><li {a11y_attrs("k2")}>{html_lib.escape(en["k2"])}</li><li {a11y_attrs("k3")}>{html_lib.escape(en["k3"])}</li></ul></section>
<section class="ap-modern-a11y-card" aria-labelledby="ap-modern-a11y-motion-title"><h3 id="ap-modern-a11y-motion-title" {a11y_attrs("motion")}>{html_lib.escape(en["motion"])}</h3><p {a11y_attrs("motion_copy")}>{html_lib.escape(en["motion_copy"])}</p></section>
<section class="ap-modern-a11y-card ap-modern-a11y-state-card" aria-labelledby="ap-modern-a11y-state-title"><h3 id="ap-modern-a11y-state-title" {a11y_attrs("state")}>{html_lib.escape(en["state"])}</h3><pre id="ap-modern-a11y-state" class="ap-modern-a11y-state" aria-live="polite" tabindex="0" {a11y_attrs("state_wait")}>{html_lib.escape(en["state_wait"])}</pre></section>
</div>
...
def runtime(slug: str) -> str:
    labels = json.dumps(LABELS, ensure_ascii=False, separators=(",", ":")).replace("</", "<\\/")
    source_id = base.STATE_SOURCE[slug]
    return f'''<script id="v172-modern-packet-label-runtime">
(()=>{{'use strict';
const COPY={labels};
...
let paintToken=0, sourceObserver=null, readinessTimer=null, readinessTries=0;
function root(){{return document.querySelector('[data-quick-assign-id]')}}
function source(){{return document.getElementById(SOURCE_ID)}}
function sourceText(){{const s=source();if(!s)return'';return String(s.value||s.textContent||'').trim()}}
...
function paint(){{
  const qa=root();const l=COPY[locale()]||COPY.en;
  if(qa){{
...
function schedulePaint(){{const token=++paintToken;const run=()=>{{if(token===paintToken)paint()}};run();queueMicrotask(run);if(window.requestAnimationFrame)requestAnimationFrame(run);setTimeout(run,20);setTimeout(run,55);setTimeout(run,95)}}
function bindStateSource(){{const s=source();if(!s||sourceObserver)return false;sourceObserver=new MutationObserver(syncAccessibleState);sourceObserver.observe(s,{{childList:true,subtree:true,characterData:true,attributes:true}});s.addEventListener?.('input',syncAccessibleState);s.addEventListener?.('change',syncAccessibleState);syncAccessibleState();return true}}
function readiness(){{readinessTries+=1;bindStateSource();schedulePaint();if((root()&&sourceText())||readinessTries>=120){{if(readinessTimer)clearInterval(readinessTimer);readinessTimer=null}}}}
function wrapApi(name){{const install=()=>{{const api=window[name];if(!api||typeof api.setLocale!=='function')return false;if(api.setLocale.__v172LabelWrapped)return true;const original=api.setLocale.bind(api);const wrapped=function(...args){{const result=original(...args);schedulePaint();return result}};wrapped.__v172LabelWrapped=true;api.setLocale=wrapped;return true}};if(!install()){{let tries=0;const timer=setInterval(()=>{{tries+=1;if(install()||tries>120)clearInterval(timer)}},25)}}}}
...
readiness();readinessTimer=setInterval(readiness,50);
}})();</script>'''


def insert_before_document_close(page: str, close_tag: str, addition: str, slug: str) -> str:
    lower = page.lower()
    if close_tag.lower() == "</head>":
        head_start = lower.find("<head")
        head_open_end = lower.find(">", head_start)
        close = lower.find("</head>", head_open_end + 1)
    else:
        close = lower.rfind(close_tag.lower())
    if close < 0:
...
def validate() -> None:
    for slug in MODERN:
        ...
        required = (
            'id="v172-modern-packet-label-runtime"', 'id="v172-modern-a11y-parity-style"',
            'class="accessibility-layer ap-modern-a11y-parity" open', 'id="ap-modern-a11y-state"',
            'id="ap-modern-a11y-live"', 'role="region"', 'syncAccessibleState', 'readinessTimer',
            '__v172LabelWrapped', 'setTimeout(run,95)', f'const SOURCE_ID={json.dumps(base.STATE_SOURCE[slug])}',
        )
        missing = [marker for marker in required if marker not in page]
        if missing:
            raise RuntimeError(f"Modern accessibility/label parity missing for {slug}: {missing}")
        packet_pos = page.rfind('id="v172-modern-packet-runtime"')
        labels_pos = page.rfind('id="v172-modern-packet-label-runtime"')
        body_pos = page.lower().rfind("</body>")
        if not (0 <= packet_pos < labels_pos < body_pos):
            raise RuntimeError(f"Modern packet label runtime is not at final document boundary: {slug}")
        style_pos = page.find('id="v172-modern-a11y-parity-style"')
        head_start = page.lower().find("<head")
        head_open_end = page.find(">", head_start)
        head_pos = page.lower().find("</head>", head_open_end + 1)
        if not (0 <= style_pos < head_pos):
            raise RuntimeError(f"Modern accessibility style is not in document head: {slug}")
    base.validate()
...
def build_site() -> None:
    ...
    for slug in MODERN:
        ...
        if 'id="v172-modern-packet-label-runtime"' in page or 'id="v172-modern-a11y-parity-style"' in page:
            raise RuntimeError(f"Modern accessibility layer would be applied twice: {slug}")
        page = replace_compact_a11y(page, slug)
        page = insert_before_document_close(page, "</head>", STYLE, slug)
        page = insert_before_document_close(page, "</body>", runtime(slug), slug)
        path.write_text(page, encoding="utf-8")
    validate()
    print("Completed v1.7.2 modern accessibility parity with lifecycle-independent state synchronization")
