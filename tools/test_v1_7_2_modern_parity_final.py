...
def main() -> int:
    release_notes = SITE / "release-notes.html"
    if not release_notes.is_file() or 'id="release-v1-7-2"' not in release_notes.read_text(encoding="utf-8"):
    ...
    for slug in ORIGINAL:
        ...
        check(f"original lab untouched by parity wrapper: {slug}", "v172-modern-parity" not in source)
    ...
    required = (
        ...
        'id="v172-modern-toolbar-runtime"', 'data-qa-modern-state',
        'data-qa-action="refresh-state"', 'id="v172-modern-packet-runtime"',
    ...
    for slug in MODERN:
        ...
        check(
            f"{slug}: canonical theme with one-time legacy migration",
    ...
    payload = {
        "harness": "tools/test_v1_7_2_modern_parity_final.py",
        "checks": len(checks),
