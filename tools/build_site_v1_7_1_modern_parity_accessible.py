...
STYLE = r'''<style id="v171-modern-a11y-parity-style">
...
function wrapApi(name){{const install=()=>{{const api=window[name];if(!api||typeof api.setLocale!=='function')return false;if(api.setLocale.__v171LabelWrapped)return true;const original=api.setLocale.bind(api);const wrapped=function(...args){{const result=original(...args);schedulePaint();return result}};wrapped.__v171LabelWrapped=true;api.setLocale=wrapped;return true}};if(!install()){{let tries=0;const timer=setInterval(()=>{{tries+=1;if(install()||tries>120)clearInterval(timer)}},25)}}}}
...
def validate() -> None:
    for slug in MODERN:
        ...
        required = (
            'id="v171-modern-packet-label-runtime"', 'id="v171-modern-a11y-parity-style"',
            ...
            '__v171LabelWrapped', 'setTimeout(run,95)', f'const SOURCE_ID={json.dumps(base.STATE_SOURCE[slug])}',
        )
...
def build_site() -> None:
    ...
    validate()
    print("Completed v1.7.1 modern accessibility parity with lifecycle-independent state synchronization")
