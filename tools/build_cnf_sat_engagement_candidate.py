...
const COPY={
 en:{title:'Watch DPLL grow and prune the search tree',cdclTitle:'Watch CDCL learn and backjump through the implication trace',help:'Every visible node comes from the existing DPLL trace. Branches create alternatives; propagation extends one branch; contradiction kills it; backtracking opens its sibling.',cdclHelp:'Every visible event comes from the bounded CDCL trace. Decisions create levels, propagation records reasons, conflict analysis learns a clause, and backjumping skips irrelevant decisions.',space:'Reference space',trace:'DPLL trace',cdclTrace:'CDCL trace',step:'Current trace step',path:'Current search path',empty:'Convert a formula to create a DPLL trace.',cdclEmpty:'Convert a formula to create a CDCL trace.',unit:'unit',pure:'pure',branchT:'try T',branchF:'backtrack F',decision:'decision',propagation:'propagation',conflict:'conflict',learn:'learn',backjump:'backjump',sat:'SAT',unsat:'conflict',legendBranch:'branch / propagation',legendConflict:'pruned conflict',legendBack:'backtrack / backjump',legendSat:'satisfying leaf'},
 zh:{title:'观察 DPLL 如何生长并剪枝搜索树',cdclTitle:'观察 CDCL 如何学习子句并沿蕴含轨迹回跳',help:'每个可见节点都来自现有 DPLL 跟踪。分支创建备选路径；传播延伸一个分支；矛盾将其剪掉；回溯再打开兄弟分支。',cdclHelp:'每个可见事件都来自有界的 CDCL 跟踪。决策建立层级，传播记录原因，冲突分析学习子句，回跳则跳过无关决策。',space:'参考空间',trace:'DPLL 跟踪',cdclTrace:'CDCL 跟踪',step:'当前跟踪步骤',path:'当前搜索路径',empty:'先转换公式以生成 DPLL 跟踪。',cdclEmpty:'先转换公式以生成 CDCL 跟踪。',unit:'单元',pure:'纯文字',branchT:'尝试 T',branchF:'回溯 F',decision:'决策',propagation:'传播',conflict:'冲突',learn:'学习',backjump:'回跳',sat:'可满足',unsat:'冲突',legendBranch:'分支 / 传播',legendConflict:'被剪掉的冲突',legendBack:'回溯 / 回跳',legendSat:'满足叶节点'},
 vi:{title:'Xem DPLL phát triển và cắt tỉa cây tìm kiếm',cdclTitle:'Xem CDCL học mệnh đề và nhảy lùi qua vết suy diễn',help:'Mỗi nút hiển thị đều đến từ vết DPLL hiện có. Phân nhánh tạo lựa chọn; lan truyền kéo dài một nhánh; mâu thuẫn cắt nhánh đó; quay lui mở nhánh anh em.',cdclHelp:'Mỗi sự kiện hiển thị đều đến từ vết CDCL có giới hạn. Quyết định tạo mức, lan truyền ghi lý do, phân tích xung đột học mệnh đề và nhảy lùi bỏ qua quyết định không liên quan.',space:'Không gian tham chiếu',trace:'Vết DPLL',cdclTrace:'Vết CDCL',step:'Bước vết hiện tại',path:'Đường tìm kiếm hiện tại',empty:'Hãy chuyển đổi một công thức để tạo vết DPLL.',cdclEmpty:'Hãy chuyển đổi một công thức để tạo vết CDCL.',unit:'đơn vị',pure:'literal thuần',branchT:'thử T',branchF:'quay lui F',decision:'quyết định',propagation:'lan truyền',conflict:'xung đột',learn:'học',backjump:'nhảy lùi',sat:'SAT',unsat:'mâu thuẫn',legendBranch:'phân nhánh / lan truyền',legendConflict:'mâu thuẫn bị cắt',legendBack:'quay lui / nhảy lùi',legendSat:'lá thỏa mãn'},
 es:{title:'Mira cómo DPLL hace crecer y poda el árbol de búsqueda',cdclTitle:'Mira cómo CDCL aprende y salta hacia atrás por la traza de implicación',help:'Cada nodo visible proviene de la traza DPLL existente. Las ramas crean alternativas; la propagación extiende una rama; una contradicción la poda; el retroceso abre su hermana.',cdclHelp:'Cada evento visible proviene de la traza CDCL acotada. Las decisiones crean niveles, la propagación registra razones, el análisis de conflicto aprende una cláusula y el salto omite decisiones irrelevantes.',space:'Espacio de referencia',trace:'Traza DPLL',cdclTrace:'Traza CDCL',step:'Paso actual de la traza',path:'Ruta de búsqueda actual',empty:'Convierte una fórmula para crear una traza DPLL.',cdclEmpty:'Convierte una fórmula para crear una traza CDCL.',unit:'unidad',pure:'literal puro',branchT:'probar T',branchF:'retroceder F',decision:'decisión',propagation:'propagación',conflict:'conflicto',learn:'aprender',backjump:'salto atrás',sat:'SAT',unsat:'conflicto',legendBranch:'rama / propagación',legendConflict:'conflicto podado',legendBack:'retroceso / salto',legendSat:'hoja satisfactoria'}
};
...
function render(){if(!P()||!$('#cnf-eq-tree'))return;const c=cp(),cdcl=$('#solverMode')?.value==='cdcl',title=cdcl?c.cdclTitle:c.title,help=cdcl?c.cdclHelp:c.help,traceLabel=cdcl?c.cdclTrace:c.trace,empty=cdcl?c.cdclEmpty:c.empty,trace=P().getTrace(),idx=Math.max(0,Math.min(P().getIndex(),Math.max(0,trace.length-1)));$('#cnf-eq-title').textContent=title;$('#cnf-eq-help').textContent=help;$('#cnf-eq-step-label').textContent=c.step;$('#cnf-eq-path-label').textContent=c.path;$('#cnf-eq-leg-branch').textContent=c.legendBranch;$('#cnf-eq-leg-conflict').textContent=c.legendConflict;$('#cnf-eq-leg-back').textContent=c.legendBack;$('#cnf-eq-leg-sat').textContent=c.legendSat;const rows=P().getRows();$('#cnf-eq-metric').textContent=`${c.space}: ${Number.isFinite(rows)?rows:'2^n'} assignments · ${traceLabel}: ${trace.length} events`;const svg=$('#cnf-eq-svg');svg.replaceChildren();if(!trace.length){$('#cnf-eq-step').textContent=empty;$('#cnf-eq-path').textContent='—';svg.setAttribute('viewBox','0 0 620 220');svg.setAttribute('aria-label',empty);return}const nodes=buildNodes(trace,idx),g=layout(nodes);svg.setAttribute('viewBox',`0 0 ${g.width} ${g.height}`);svg.setAttribute('aria-label',`${title}. ${c.step}: ${idx+1}/${trace.length}.`);const rootY=22;for(const rootId of g.roots){const n=nodes[rootId];svg.appendChild(svgEl('path',{d:`M ${g.rootX} ${rootY} L ${n.x} ${n.y-24}`,class:'cnf-eq-edge'}))}for(const n of nodes)if(n.parentId>=0){const p=nodes[n.parentId];svg.appendChild(svgEl('path',{d:`M ${p.x} ${p.y+24} C ${p.x} ${p.y+50}, ${n.x} ${n.y-50}, ${n.x} ${n.y-24}`,class:'cnf-eq-edge'}))}for(const n of nodes){const group=svgEl('g',{class:`cnf-eq-node ${n.id<idx?'past':''} ${n.id===idx?'current':''} ${n.item.action==='sat'?'sat':''} ${(n.item.action==='unsat'||n.item.action==='conflict')?'unsat':''} ${(n.item.action==='branch-'||n.item.action==='backjump')?'backtrack':''}`.trim(),transform:`translate(${n.x} ${n.y})`});const titleEl=svgEl('title');titleEl.textContent=`${n.id+1}. ${n.item.detail||eventLabel(n.item,n.parentId>=0?nodes[n.parentId].item:null)}`;group.append(titleEl,svgEl('circle',{r:24}));const text=svgEl('text',{y:4});const parent=n.parentId>=0?nodes[n.parentId].item:null;let label=eventLabel(n.item,parent);if(label.length>16)label=label.slice(0,15)+'…';text.textContent=label;group.appendChild(text);svg.appendChild(group)}const current=nodes.at(-1),parent=current.parentId>=0?nodes[current.parentId].item:null;$('#cnf-eq-step').textContent=`${idx+1}/${trace.length} · ${eventLabel(current.item,parent)} · ${current.item.detail||''}`;$('#cnf-eq-path').textContent=ancestors(nodes,current.id).map(n=>eventLabel(n.item,n.parentId>=0?nodes[n.parentId].item:null)).join(' → ')}
