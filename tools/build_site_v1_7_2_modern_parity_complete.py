...
"""Complete v1.7.2 modern-lab parity composition.
...
import build_site_v1_7_2_modern_parity_final as base
...
STYLE = r'''<style id="v172-modern-quick-assign-parity-style">
...
def packet_runtime(slug: str) -> str:
    ...
    return f'''<script id="v172-modern-packet-runtime">
...
function refresh(){{if(!state)return;const value=String(source?.value||source?.textContent||'').trim();state.textContent=value||labels().empty}}
...
def build_site() -> None:
    ...
    for slug in MODERN:
        ...
        if 'id="v172-modern-quick-assign-parity-style"' not in page:
            page = page.replace('</head>', STYLE + '\n</head>', 1)
        page = patch_markup(page, slug)
        page = page.replace('</body>', packet_runtime(slug) + '\n</body>', 1)
        path.write_text(page, encoding="utf-8")
    validate()
    print("Completed v1.7.2 modern Quick Assign parity with state snapshot, refresh, packet copy, and packet-only print")
